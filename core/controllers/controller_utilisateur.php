...
if ($page == 'lost-password') {
	...
	if (DEV_MODE) {
		if ($action == 'ask-renew' && $referer != EXTERNAL_URL . getUrl('utilisateur', 'connexion-inscription', 'view')) {
			header('Location: ' . getUrl('utilisateur', 'connexion-inscription', 'view'));
		}
	}
}
...
$includedCssScripts = [['href' => HTML_PUBLIC_STYLES_DIR . 'fontawesome/css/fontawesome.min.css'], ['href' => HTML_PUBLIC_STYLES_DIR . 'fontawesome/css/solid.min.css']];
if ($page == 'mon-compte' || $page == 'tableau-de-bord') {
	$includedCssScripts[] = ['href' => HTML_PUBLIC_STYLES_DIR . 'cropper.min.css'];
	$includedCssScripts[] = ['href' => 'https://cdn.jsdelivr.net/npm/intl-tel-input@' . INTL_TEL_INPUT_VERSION . '/build/css/intlTelInput.css'];
}
if ($page == 'messagerie') {
	$includedCssScripts[] = ['href' => 'https://cdn.jsdelivr.net/npm/quill@2.0.2/dist/quill.snow.css'];
}
require_once 'core/views/template/header.phtml';
switch ($page) {
	case 'connexion-inscription': {
			$includedJSScripts = [HTML_PUBLIC_SCRIPTS_DIR . 'connexion-inscription.js', HTML_PUBLIC_SCRIPTS_DIR . 'tabs-navs.js', HTML_PUBLIC_SCRIPTS_DIR . 'google_facebook_functions.js'];
			require_once 'core/views/view_connexion_inscription.phtml';
			break;
		}
	case 'lost-password': {
	...
	case 'messagerie': {
			if (!isset($userLogged) or $userLogged->getId() == 0) {
				...
				$includedJSScripts = [ //HTML_PUBLIC_LIBS_DIR.'tmce/tinymce.min.js',
					...
					HTML_PUBLIC_SCRIPTS_DIR . "js-messagerie.js"
				];
